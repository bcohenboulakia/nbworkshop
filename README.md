...
The student version is created in the same directory as the original Notebook, with solutions replaced by placeholders and all execution traces (including calculation results and cell execution counters) automatically removed. The summary is sent to the standard output. In this summary, Notebooks are refered by absolute path if not in the hierarchy of the current working directry.
